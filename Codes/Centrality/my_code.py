...
xtcname = 'temp_traj/md_0_1.xtc'
groname = 'temp_traj/md_0_1.gro'

print("filenames read")
#output filenames
extnname='.dat'
file1name='Network_Stats_'+extnname
file2name='molhb_histogram_'+extnname
file3name='hbweight_histogram_'+extnname
file5name='Pathlength_histogram_'+extnname
file6name='PerFrame_HistCount_'+extnname
file7name='Centrality_histogram_'+extnname
#
...
btbin_width = 0.005
clbin_width = 0.01
kd_tree_threshold = 0.35
#

# Defining some centrality functions (with optimisation)
def calculate_closeness_centrality(graph):
    return nx.closeness_centrality(graph, distance="weight", approximation=True)

def calculate_betweenness_centrality(graph):
    return nx.betweenness_centrality(graph, weight="weight", approximation=True)
...
for frame in range(0,n_frame):
    kdtree = cKDTree(coord[frame])  # assuming that coord is a 3D numpy array where coord[frame, i] gives the coordinates of atom i in the given frame
	count =0
	...
	for donors_index in donors:
        index1 = donor_index
		rid1 = topol.atom(index1).residue.index
		potential_acceptors_indices = kdtree.query_ball_point(coord[frame][index1], r=kd_tree_threshold)
        ...
        for acceptor_index in potential_acceptors_indices:
			...
			if rid1 != rid2:
				...
				if DAdist_sq <= 0.1225:
					# Combine the calculations for angle_h1 and angle_h2 into a loop
                    for k in range(1, 3):
                        angle_h = angle(frame, index1 + k, index1, index2)
                        if angle_h <= 30:
                            count += 1
                            wt = 1.0
    ...
    path = nx.floyd_warshall(hbnet)   ## Applying Floyd Warshall on the graph
    print("after Floyd-Warshall path for 1st network")
    print("--- %s seconds ---" % (time.time() - start_time))
    ...
    for nd in hbnet.nodes:  ## No need to create a list here (time saving)
    ...
    # Calculate mecount1 and mecount2 using NetworkX functions
    degree_sequence = list(hbnet.degree())
    degree_hist = nx.degree_histogram(hbnet)

    mecount1 = sum(count for degree, count in enumerate(degree_hist) if degree > 1)
    mecount2 = sum(count for degree, count in enumerate(degree_hist) if degree > 2)
	
    #hbnet node loop (Removed redundant calculation)
    for ss, Atar in path.items():  # Changed to use items() directly
        for tt, path_ss_tt in Atar.items():  # Changed to use items() directly
            nconn = len(path_ss_tt) - 1  # Removed separate calculation
            if nconn > 0:
                pcount += 1
                if nconn < mpl:
                    path_hist[nconn] += 1
                else:
                    print("Total pathlength %d exceeds maximum pathlength of %d" % (nconn, mpl))
                    # print frame, ss, tt, nconn, path_ss_tt
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(calculate_closeness_centrality, hbnet),
                executor.submit(calculate_betweenness_centrality, hbnet)]
        
        clcen1, btcen1 = futures[0].result(), futures[1].result()
