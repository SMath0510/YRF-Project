...
class CustomDistanceKDTree:
    def __init__(self, points, custom_distance_func):
        self.k = len(points[0])  # Dimensionality of the points
        self.root = self.build_tree(points, custom_distance_func)

    def build_tree(self, points, custom_distance_func, depth=0):
        if len(points) == 0:
            return None
        ...
        left_subtree = self.build_tree(points[:median], custom_distance_func, depth + 1)
        if(median + 1 == len(points)):
            right_subtree = None
        else:
            right_subtree=self.build_tree(points[median + 1:], custom_distance_func, depth + 1)
        return KDNode(
            point=points[median],
            left=left_subtree,
            right= right_subtree
    ...
    def query_radius(self, query_point, r):
        result = []
        self._query_radius(self.root, query_point, r, 0, result)
        return result

    def _query_radius(self, node, query_point, r, depth, result):
        if node is not None:
            axis = depth % self.k
            distance = custom_distance(query_point, node.point)

            if distance <= r:
                result.append(node.point)

            if query_point[axis] - r < node.point[axis]:
                self._query_radius(node.left, query_point, r, depth + 1, result)
            if query_point[axis] + r > node.point[axis]:
                self._query_radius(node.right, query_point, r, depth + 1, result)
...
file10.write(f"Graph Construction, Shortest Path, Centrality\n")
...
for frame in range(0,num_iter):
    print(f"{frame}/{num_iter}")
    local_start_time = time.time()
    print(box_len[frame], coord[frame])
    continue
    # kd_tree_base = []
    # for acceptor_idx in acceptors:
    #     kd_tree_base.append(coord[frame][acceptor_idx])
        
    if optimize_loops: 
        # kdtree = KDTree(coord[frame], metric = 'euclidean') 
        kdtree = CustomDistanceKDTree(coord[frame], custom_distance_func=custom_distance)
    count =0

    ## Applying an optimisation that converts the code from O(n2) to O(nlogn)
    for donor_index in [15, 270]:
        index1 = donor_index
        ...
        if optimize_loops:
            ...
            potential_acceptors_indices = kdtree.query_radius(coord[frame][index1], kd_tree_threshold)
            # query_point = np.array([coord[frame][index1]])
            # potential_acceptors_indices = kdtree.query_radius(query_point, r=kd_tree_threshold)[0]
            # _, potential_acceptors_indices = kdtree.query(query_point, k=5000)

            # Filter out indices not in acceptors
            print(potential_acceptors_indices)
            # potential_acceptors_indices = [item for item in potential_acceptors_indices if item in acceptors]
            pass

        else:
            potential_acceptors_indices = [acc_index for acc_index in acceptors if distance_sq(frame, index1, acc_index) <= kd_tree_threshold*kd_tree_threshold]
        ...
        if donor_index == 15:
            print(my_list)
            print([distance_sq(frame, donor_index, i) for i in my_list])
            print(potential_acceptors_indices)
            
            print([(i, distance_sq(frame, donor_index, i)) for i in potential_acceptors_indices])
            print(distance_sq(frame,donor_index,270))
    #end of donor loop
    print(coord[frame][15], coord[frame][270])
    #in_degree = number of h-bonds accepted by the molecule
    #out_degree = number of h-bonds donated by the molecule
    # print(f'Potential List: {potential_acceptors_indices}\n\nActual List: {my_list}\n')
    if cmd_print:
        print("Network created for frame %d"%(frame))
        print("--- %s seconds ---" % (time.time() - start_time))
        
    # for item in my_list:
    #     if item not in potential_acceptors_indices:
    #         print(f"WTF{frame} :: {item}")
