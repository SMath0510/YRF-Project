...
groname = 'bulk_tip3p_all.gro'

print("filenames read")

#output filenames
extnname='.dat'
file1name='shaun_other_outputs/Network_Stats_'+extnname
file2name='shaun_other_outputs/molhb_histogram_'+extnname
file3name='shaun_other_outputs/hbweight_histogram_'+extnname
file5name='shaun_other_outputs/Pathlength_histogram_'+extnname
file6name='shaun_other_outputs/PerFrame_HistCount_'+extnname
file7name='shaun_other_outputs/Centrality_histogram_'+extnname
#
...
rid1 = topol.atom(1).residue.index

file1 = open("temp.txt", 'w')
file1.write(f"info = \n{info}\n\ncoord = \n{coord}\n\ndonors = \n{donors}\n\nbox = {box}\n\n")

file1.write(f"Printing the atomic details\n\n")

file1.write(f"The residue names\n\n")
for i in range (0,nres):
	rname = topol.residue(i).name
	file1.write(f"Index: {i} -> rname: {rname}\n")
	
file1.write(f"\nThe topol.atom = \n{topol.atom(1).residue}\n\n") 
#file1.write(f"The index and the rids\n")
for index1 in donors:
	rid1 = topol.atom(index1).residue.index
	for index2 in acceptors:
		rid2 = topol.atom(index2).residue.index
		#file1.write(f"Index: ({index1}, {index2}) -> rid: ({rid1}, {rid2})\n")
